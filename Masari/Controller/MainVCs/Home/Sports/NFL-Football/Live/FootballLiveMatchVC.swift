...
class FootballLiveMatchVC: UIViewController {
    ...
    func getFootBallLiveMAtch(){
        showLoader()
        FootballManager.instance.getFootballSports { [weak self](success, liveMatch, error) in
            if success {
                self?.hideLoader()
...
extension FootballLiveMatchVC: UITableViewDelegate, UITableViewDataSource {
